...
var game = {
    ...
    start: function() {
        ...
        myGamePiece_DrawingComet = new drawComet();
    },
    clear: function() {
...
var cometShown = false; // Flag to check if the image is loaded
var cometActive = false; // Flag to check if the image is loaded
var cometX = -1000;
var cometY = getRandomInt(25, 570);
var cometSpeed = speed_g/2;
var cometInterval = time_to_refuel; // in milliseconds
var lastCometTime = 0;
var drawing = 0;
var tirMissile = 0;
var impactMissile = 0;
...
var element = 0;
var interval_to_refuel = 10; // users have 10 seconds to refuel
var time1 = time_to_refuel + time_to_refuel*0.1;
var time2 = time1 + time_to_refuel - time_to_refuel*0.2;
var time3 = time2 + time_to_refuel - time_to_refuel*0.1;
var time4 = time3 + time_to_refuel + time_to_refuel*0.3;
var time5 = time4 + time_to_refuel - time_to_refuel*0.3;
var time6 = time5 + time_to_refuel + time_to_refuel*0.2;

var time_to_refuel_list = [time1, time2, time3, time4, time5, time6];
...
function message() {
    ...
    this.draw = function() {
        ...
        // Code managing to show the fuel available
        ...
        if (new Date().getTime() > myGamePiece_Time.time + 1000) {
            temps_essence += 1;
            console.log('temps essence ' +temps_essence + ' temps rocher ' + time_to_refuel_list[element])
            // if time to refuel is passed
            if (temps_essence > (time_to_refuel_list[element]) && myGamePiece_DrawingComet.cometX < 0) {
                ...
                this.affichage_plein = 2000;
                setTimeout(function() {
                    myGamePiece_DrawingComet.cometX = -1000;
                }, 10000);
                cometShown = false;
            }
            myGamePiece_Time.time += 1000;
            this.longueur -= 150/time_to_refuel_list[element];
            // code that is not useful for task and event based version
            if (temps_essence > (time_to_refuel_list[element]/2) && temps_essence < (time_to_refuel_list[element]/1.2)) {
                this.color = "#FFFF00";
            ...
            // time to display Appuyez sur touche barre espace pour refuel
            }else {
                if ( (temps_essence > time_to_refuel_list[element] - interval_to_refuel) && (temps_essence < time_to_refuel_list[element]) ) {
                    this.color = "#FF0000";
                    if (myGamePiece_Affichage.countPressPlein == 0) {
                        ...
                        cometActive = true;
                        if (cometActive && !cometShown){
                            myGamePiece_DrawingComet.cometX = 1000;
                            myGamePiece_DrawingComet.cometY = getRandomInt(25, 570);
                            cometActive = false;
                            cometShown = true;
                        }
                    }
        ...
        // if time to refuel and user refueled
        if ((temps_essence > (time_to_refuel_list[element]-interval_to_refuel) && myGamePiece_DrawingComet.cometX > 0) && (game.keyCode3 == true) && (myGamePiece_Affichage.keypress == false)) {
            ...
            // if not time to refuel but user try to refuel
            if ((temps_essence > 0 && temps_essence < (time_to_refuel_list[element]-interval_to_refuel)) && (game.keyCode3 == true) && (myGamePiece_Affichage.keypress == false)) {
                var b = new Date().getTime() - myGamePiece_Time.timeStart;
                essenceWrong.push(b);
                myGamePiece_Affichage.keypress = true;
                setTimeout("keyRestore()", 1000);
...
function drawComet() {
    ...
    this.draw = function() {    
        context = game.context;
        this.cometX -= cometSpeed;
        context.drawImage(cometImage, this.cometX,  this.cometY , 100, 50);        
    }
}
