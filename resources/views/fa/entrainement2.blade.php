...
<body onload="startGame()" style="background-color:black; direction: rtl !important">
  <form id = "bouton_consigne" action="instruction3" method="POST">
    @csrf
    ...
    start : function() { // fonction start
		...
		function checkTime(){
    		if(game.redirect == false) {
    		    ...
    		    Start = function(){setTimeout("stop()", 1);
    		        stop = function(){
    		            $("#bouton").click();
    		        }
    		    }
...
this.timeStart = timeStart;
  ...
  function message() {
        this.x = 550;
        this.text1 = 'کلید "F" را فشار دهید. ';
	...
	function Evenement() {
	    this.update = function(){
        	...
        	if (game.keyCode1 == true && myGamePiece_Missile.nouveau_tir == false) {
        		...
        		if (game.tirs < 1 ) {
        		    game.tirs += 1;
            		myGamePiece_Affichage.text = 'یکبار دیگر با استفاده از کلید "F" شلیک کنید.';
            		myGamePiece_Affichage.x = 650;
        		}
        		else if (game.tirs >= 1 ) {
            		myGamePiece_Affichage.text = 'یکبار دیگر با استفاده از کلید "F" شلیک کنید.';
            		myGamePiece_Affichage.x = 650;
...
function updateGame() {
	...
	context.font= "25px Verdana";
	context.fillStyle = "#FFFFFF";
	context.fillText(myGamePiece_Affichage.text, myGamePiece_Affichage.x, 330);
