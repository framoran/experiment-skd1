...
<body onload="startGame()" style="background-color:black;">
    <form id = "bouton_consigne" action="entrainement2" method="POST">
      @csrf
...
function drawCenteredText(context, text, yPosition, maxWidth) {
    context.font = "25px Verdana"; // Ensure the font is set for consistent measurements
    var textWidth = context.measureText(text).width;

    // Check if the text needs to be split into two lines
    if (textWidth > maxWidth) {
        // Find the middle of the text and split it at the nearest space
        var words = text.split(' ');
        var line1 = "";
        var line2 = "";
        var line1Width = 0;

        // Loop through the words to split the text
        for (var i = 0; i < words.length; i++) {
            var testLine = line1 + words[i] + " ";
        ...
        // Calculate X positions to center each line
        var canvasCenterX = game.canvas1.width / 2;
        var line1X = canvasCenterX - line1Width / 2;
        var line2Width = context.measureText(line2).width;
        var line2X = canvasCenterX - line2Width / 2;

        // Draw both lines centered
        context.fillText(line1, line1X, yPosition);
        context.fillText(line2, line2X, yPosition + 30);
    } else {
        // Center the text on one line
        var textX = (game.canvas1.width - textWidth) / 2;
        context.fillText(text, textX, yPosition);
    }
}
    ...
    start : function() { // fonction start
		...
		function time(){
		    if (game.checkUpdate == false){
				...
				var text = "Oikein hyvä! Nyt voit harjoitella ohjusten laukaisemista painamalla F-kirjainta näppäimistöltä.";
				context.font = "25px Verdana"; // Set the font style to ensure consistent measurements
				var textWidth = context.measureText(text).width;

				if (textWidth > 800) {
					// Split the text into two lines if the width exceeds 800 pixels
					var text1 = "Oikein hyvä! Nyt voit harjoitella ohjusten laukaisemista";
					var text2 = "painamalla F-kirjainta näppäimistöltä.";
					var canvasCenterX = game.canvas1.width / 2; // Center X of the canvas

					// Calculate the position to center the text
					var text1Width = context.measureText(text1).width;
					var text2Width = context.measureText(text2).width;
					var text1X = (canvasCenterX - text1Width / 2);
					var text2X = (canvasCenterX - text2Width / 2);

					// Draw the two lines centered
					context.fillText(text1, text1X, 300);
					context.fillText(text2, text2X, 350);
				} else {
					// Center the text on one line
					var textX = (game.canvas1.width - textWidth) / 2;
					context.fillText(text, textX, 300);
				}    		    game.redirect = true;
		    }
		}
		...
		function checkTime(){
    		if(game.redirect == false) {
    		    ...
    		    Start = function(){setTimeout("stop()", 3000);
    		        stop = function(){
    		            $("#bouton").click();
    		        }
    		    }
...
this.timeStart = timeStart;
  ...
  // OBJET message
  function message() {
	this.text1 = "Pidä oikealle osoittavaa nuolinäppäintä painettuna,";
    this.text2 = "Pidä vasemmalle osoittavaa nuolinäppäintä painettuna,";
    this.text3 = "Pidä ylöspäin osoittavaa nuolinäppäintä painettuna,";
    this.text4 = "Pidä alaspäin osoittavaa nuolinäppäintä painettuna,";
    this.text_ligne = "kunnes avaruusalus osuu ruudun reunaan.";
    this.textCheck1 = false;
    this.textCheck2 = false;
    this.textCheck3 = false;
    this.textCheck4 = false;
    this.textCheck5 = false;
	...
	function Evenement() {
	    this.update = function(){
        	...
        	if (myGamePiece_Vaisseau.x < 1) {
        	    ...
        	    if (myGamePiece_Affichage.textCheck2 == true){
        	        myGamePiece_Affichage.text = myGamePiece_Affichage.text3;
        	        myGamePiece_Affichage.textCheck3 = true;
        	    }
        	}

        	if (myGamePiece_Vaisseau.x > 800) {
        	    myGamePiece_Vaisseau.x = 800;
        	    myGamePiece_Affichage.text = myGamePiece_Affichage.text2;
        	    myGamePiece_Affichage.textCheck2 = true;
        	}

        	if (myGamePiece_Vaisseau.y > 500) {
        	    myGamePiece_Vaisseau.y = 500;
        	    if (myGamePiece_Affichage.textCheck4 == true){
        	        myGamePiece_Affichage.text = myGamePiece_Affichage.text1;
        	    }
        	    if (myGamePiece_Affichage.textCheck5 == true){
                game.checkUpdate = true;
        	    }
        	}

        	if (myGamePiece_Vaisseau.y < 25) {
        	    myGamePiece_Vaisseau.y = 25;
        	    myGamePiece_Affichage.textCheck2 = false;
        	    if (myGamePiece_Affichage.textCheck3 == true){
        	        myGamePiece_Affichage.text = myGamePiece_Affichage.text4;
        	        myGamePiece_Affichage.textCheck5 = true;
        	    }
        	}

        	if (game.key && game.key == 70 && myGamePiece_Missile.nouveau_tir == false) {
...
function updateGame() {
	...
	context.font= "25px Verdana";
	context.fillStyle = "#FFFFFF";
	context.fillText(myGamePiece_Affichage.text, 190, 250);
	context.fillText(myGamePiece_Affichage.text_ligne, 190, 310);
